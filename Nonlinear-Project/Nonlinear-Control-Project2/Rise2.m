...
[t,STATES] = ode45(@(t,X) composite_rise2(t,X,theta),[0 tf],X0,opts);
...
e2_0     = [3;2];
thetahat_0 = [1;1;1;1;1];

% Select gains for controller
K        = 200; % Enter a number
K2       = 200;
a1       = 400; % Enter a number
a2       = 400;
...
% Design RISE term
Rise_intergrate_1_Dot = (K+1)*a2*e2 + Beta*sign(e2);
miu1 = (K+1)*e2 - (K+1)*e2_0 + Rise_intergrate_1;
...
thetahat_part_2_dot = gamma*Ydf_dot'*E;
thetahat =  thetahat_part_2 + thetahat_part_1 + thetahat_0;


%%
Ydf_dot = reshape(Ydf_dot',[10,1]);
XDot        = [e1Dot;e2Dot;rDot;thetaHatDot;uf_dot;Ydf_dot; ...
    Rise_intergrate_1_Dot;Rise_intergrate_2_Dot;thetahat_part_2_dot];